...
class TestAuthElements(TestCase):
    ...
    def test_register_wrong_pw(self):
        ...
        """
        rand_pw = lambda: ''.join([random.choice(string.ascii_letters) for _ in range(8)])
    ...
    def test_login_wrong(self):
        ...
        response = self.client.post('/accounts/login/', data={'username': self.user_data['username'], 'password': rand()})
        self.assertNotEqual(response.status_code, 302)
        form = response.context['form']
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors['__all__'][0], 'Please enter a correct username and password. Note that both fields may be case-sensitive.')
